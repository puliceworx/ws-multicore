...
LOG_MODULE_REGISTER(cpunet, CONFIG_BT_LOG_LEVEL);
...
{
  //LOG_INF("Radio_IRQHandler");
  //nrf_gpio_pin_toggle(RADIO_NOTIFICATION_PIN);
}

void setup_radio_notifications(void)
{
  int err = 0;
...
//                                                TIMESLOT_IRQN);
  if (err) 
  {
    LOG_INF("mpsl_radio_notification_cfg_set() returned %d", err);
  }
  LOG_INF("rn ret %d", err);
}
